...
def check_permissions(user, method, permission_to):
    ...
    for group in groups:
        ...
        for perm in permissions:
            if Permission.objects.filter(id=perm['permission_id'], codename=required_permission).exists():
                return True


class EmployessPermission (permissions.BasePermission):
...
class GroupPermissionPermission (permissions.BasePermission):
    message = "O funcionario nao tem permissão para gerenciar as permissões"
